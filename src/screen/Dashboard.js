...
export default class Dashboard extends Component {
  ...
  panggilDelayData = () => {
    this.getPatient_id().then(pmi_no => {
      ...
      fetch("http://172.30.20.209:8082/show_no/" + pmi_no)
  ...
  send = async () => {
    await this.askPermissionsAsync(),
      Notifications.presentLocalNotificationAsync({
        ...
        body: "Currently Serving Number is : 1001",
        // data: {
        //   hfc_cd_name: JSON.stringify(responseJson.result[0]["hfc_cd_name"])
        // },
        android: {
        ...
        <View style={stylesDash.contena}>
          <TouchableOpacity style={stylesDash.card}>
            <View style={stylesDash.container1}>
              ...
              <Text style={stylesDash.nore1}>{this.state.hfc_cd_name}</Text>
              <Text style={stylesDash.nore1}>{this.state.discipline_name}</Text>
              <Text style={stylesDash.nore1}>{this.state.queue_name}</Text>
