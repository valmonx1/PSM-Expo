...
export default class Patient extends Component {
  ...
  componentDidMount() {
    fetch("http://172.30.20.209:8082/hospital")
  ...
  GetPickerSelectedItemValue = async () => {
    ...
    if (pmi_no == "") {
      ...
      fetch("http://172.30.20.209:8082/select_pms_last_queue", {
        method: "POST",
        headers: {
  ...
  getDiscplineValue = selectedHospId => {
    ...
    //pegang id
    fetch("http://172.30.20.209:8082/discipline/" + selectedHospId)
      .then(response => response.json())
      .then(responseJson => {
        this.setState({
  ...
  getQueueValues = selectedDiscpId => {
    ...
    fetch(
      "http://172.30.20.209:8082/queue_name/" +
        this.state.hfc_cd +
        "/" +
